...
## Laravel/PHP Exemplos

### Service Layer (guest-list-pro)
```php
...
class GuestService {
    ...
    public function create(array $data): Guest {
        $normalizedDoc = $this->validator->normalize($data['document']);
        
        if ($this->guests->existsByDocument($data['event_id'], $normalizedDoc)) {
            throw new DuplicateGuestException();
        }

        $guest = $this->guests->create([
            ...$data,
            'document_normalized' => $normalizedDoc,
        ]);

        $this->events->dispatch(new GuestCreated($guest));
...
```php
// tests/Unit/GuestServiceTest.php
...
class GuestServiceTest extends TestCase {
    ...
    public function test_create_throws_duplicate_exception(): void {
        $guests = Mockery::mock(GuestRepositoryInterface::class);
        $guests->shouldReceive('existsByDocument')
            ->with(1, '12345678900')
            ->once()
            ->andReturn(true);

        $validator = Mockery::mock(DocumentValidationService::class);
        $validator->shouldReceive('normalize')
            ->with('123.456.789-00')
            ->andReturn('12345678900');

        $service = new GuestService($guests, $validator, Mockery::mock(EventDispatcher::class));

        $this->expectException(DuplicateGuestException::class);
        $service->create([
            'event_id' => 1,
            'name' => 'John Doe',
...
### Artisan Commands do Projeto
```bash
# Testing
php artisan test --filter=GuestServiceTest
php artisan test --filter=ApprovalRequestServiceTest
