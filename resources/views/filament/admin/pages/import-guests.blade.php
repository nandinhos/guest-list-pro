<x-filament-panels::page>
    <div class="space-y-6">
        <x-filament::section>
            <x-slot name="heading">
                Importar Convidados (Administração)
            </x-slot>
            <x-slot name="description">
                Importação em massa para qualquer evento e promoter do sistema.
            </x-slot>
            ...
            <div class="mt-6">
                ...
                <div x-show="$wire.activeTab === 'file'" class="space-y-6">
                    <form wire:submit="import" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <x-filament::input.wrapper label="Evento" required>
                                <x-filament::input.select wire:model.live="eventId" required>
                            ...
                            <x-filament::input.wrapper label="Setor" required>
                                <x-filament::input.select wire:model="sectorId" required :disabled="!$this->eventId">
                                    <option value="">Selecione...</option>
                            ...
                            <x-filament::input.wrapper label="Responsável" required>
                                <x-filament::input.select wire:model="promoterId" required>
                                    <option value="">Selecione...</option>
                ...
                <div x-show="$wire.activeTab === 'text'" class="space-y-6" x-cloak>
                    <form wire:submit="importFromText" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <x-filament::input.wrapper label="Evento" required>
                                <x-filament::input.select wire:model.live="textEventId" required>
                                    <option value="">Selecione...</option>
                                    @foreach($this->events as $id => $name)
                                        <option value="{{ $id }}">{{ $name }}</option>
                                    @endforeach
                            ...
                            <x-filament::input.wrapper label="Setor" required>
                                <x-filament::input.select wire:model="textSectorId" required :disabled="!$this->textEventId">
                                    <option value="">Selecione...</option>
                            ...
                            <x-filament::input.wrapper label="Responsável" required>
                                <x-filament::input.select wire:model="textPromoterId" required>
                                    <option value="">Selecione...</option>
                                    @foreach($this->promoters as $id => $name)
                                        <option value="{{ $id }}">{{ $name }}</option>
                                    @endforeach
