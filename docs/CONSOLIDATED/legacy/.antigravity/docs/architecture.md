...
| Widget | Descrição | Queries |
|--------|-----------|---------|
| StatsOverview | Métricas gerais | 4 |
| RecentGuests | Últimos guests | 1 |
| ApprovalsPending | Aprovações pendentes | 1 |

### Promoter

| Widget | Descrição | Queries |
|--------|-----------|---------|
| PromoterStats | Métricas do promoter | 3 |
| PromoterPerformanceChart | Gráfico de performance | 2* |
| MyGuestsTable | Meus convidados | 1 |
...
| Widget | Descrição | Queries |
|--------|-----------|---------|
| CheckinStats | Métricas de check-in | 3 |
| SuspiciousCheckins | Tentativas suspeitas | 1* |
| RecentCheckins | Check-ins recentes | 1 |

*Candidato a eager loading

### Bilheteria

| Widget | Descrição | Queries |
|--------|-----------|---------|
| SalesStats | Métricas de vendas | 3 |
| TodaySales | Vendas de hoje | 1 |
| CashierBalance | Saldo do caixa | 1 |
...
```
1. Promoter cria Guest
   └─> GuestObserver normaliza dados
       └─> Verifica duplicidade por documento
           ├─> Duplicado: Bloqueia criação
           └─> Único: Salva guest

2. Admin visualiza pending
   └─> Pode aprovar/rejeitar
       └─> ApprovalRequestService processa
           └─> Notifica requester
```
...
```
1. Validator busca guest
   └─> GuestSearchService.search()
       └─> Retorna matches

2. Validator confirma check-in
   └─> Verifica se já fez check-in
       ├─> Sim: Registra CheckinAttempt (duplicate)
       └─> Não: Atualiza guest.checked_in_at
           └─> Registra CheckinAttempt (success)
```

### Fluxo de Aprovação

```
1. Promoter cria ApprovalRequest
   └─> Tipo: GuestInclusion ou EmergencyCheckin
       └─> Status: Pending
           └─> Define expires_at
...
3. Expiração automática
   └─> Job verifica expires_at
       └─> Marca como Rejected se expirado
```
